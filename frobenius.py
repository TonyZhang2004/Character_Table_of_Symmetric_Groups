...
def convert_dict_partitions_to_list(n: int):
    """
    Uses the builtin partition module and
    convert that to a python list
    """
...
def prod_power_sum(j: int, k: int):
    """
...
def frobenius(irrep: List[int], conj_class: List[int]):
    """
    calculate the product of the polynomial and get the character
    value using the monomial that corresponds to the Specht Module
    """
    kv = {}
    ...
    p_conj_class = R.mul(
        [
            R.mul([prod_power_sum(num, len(irrep)) for _ in range(freq)])
            for num, freq in kv.items()
        ]
    )
...
def get_character_table(n: int):
    """
    Get the final character table by computing
    each slot using the Frobenius formula.
    """
